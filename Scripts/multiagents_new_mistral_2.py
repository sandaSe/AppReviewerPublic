...
def classifier_agent(state: ReviewState) -> ReviewState:
    ...
    prompt = f"""
                    ...
                    - NOTE: If the review expresses **dissatisfaction due to lack of a feature**, even if it sounds like a UX complaint, it likely belongs here.

                    3. **UserExperience**
                    - The review provides detailed feedback on **usability**, **interface**, **layout**, **design**, or how a feature made them feel.
                    - Look for mentions of ease of use, look and feel, UI/UX design, frustration with interaction, or satisfaction/dissatisfaction that is **not due to bugs or missing features**.
                    - DO NOT select this if the review is just requesting a feature or reporting a bug.

                    4. **Rating**
                    - The review is very brief and mainly reflects an overall impression or emotion without specific details.
                    - Often tied closely to the star rating itself (e.g., “Great app!”, “Hate it”, “Nice work”).
                    - Use this ONLY when the review lacks enough detail for other categories.

                    **VERY IMPORTANT**:
                    - Use the **rating value ({rating})** to guide your decision. A 1-star with a vague complaint may be UX or Bug; a 5-star with “Nice” is likely Rating.
                    - DO NOT label reviews as UserExperience unless they give **specific design or usability feedback**.

                    ---

                    Instructions:
                    1. Decide the best matching category: "Bug", "Feature", "UserExperience", or "Rating".
                    2. Justify your choice with a brief reason.
                    3. Give your confidence score (between 0.0 and 1.0).

                    Respond ONLY in this JSON format:
                    {{"label": "<Bug/Feature/UserExperience/Rating>", "confidence": float between 0 and 1, "reason": "short explanation"}}
                    """
...
def uncertainty_agent(state: ReviewState) -> ReviewState:
    ...
    if confidence < 0.7 and state.get("uncertainty_count", 0) < 3:
        ...
        prompt = f"""
                        ...
                        Previous Reasoning: "{pred.get('reason', '')}"

                        Please re-analyze the review content and rating to provide a revised label and explanation.

                        Review: "{review}"
                        User Rating: {rating}

                        Use these definitions:
                        - **Bug**: Mentions crashes, errors, glitches, or malfunctions.
                        - **Feature**: Suggests new functionalities or improvements.
                        - **UserExperience**: Focuses on satisfaction, frustration, ease of use, design, or UI/UX.
                        - **Rating**: Expresses a quick opinion (positive or negative) with little or no context or detail.

                        **Pay close attention to whether the review is vague or detailed, and how the star rating supports or contradicts the content.**

                        ### Instructions:
                        1. Read the review + rating again with fresh perspective.
                        2. Consider the prior label and explanation, but don't be bound by it.
                        3. Output your final label, short explanation, and confidence.

                        ### Respond in JSON ONLY:
                        {{"label": "<Bug|Feature|UserExperience|Rating>", "confidence": float between 0 and 1, "reason": "brief justification"}}
                        """
...
def optimizer_agent(state: ReviewState) -> ReviewState:
    ...
    if confidence < 0.7:
        ...
        prompt = f"""
        You are an optimizer agent. Integrate the history of decisions below to give your final classification.
        Agent trace: {trace_str}

        Your task is to provide a final judgment by integrating:
        - The review content
        - The user rating
        - The classifier's output and reasoning
        - The uncertainty agent's explanation

        Carefully consider all information above. Select the most appropriate category ("Bug", "Feature", "UserExperience", or "Rating"), provide a concise explanation for your choice, and estimate your confidence (between 0 and 1).

        Review: "{review}"
        User rating: {rating}

        Respond with JSON:
        {{"label": "<one of Bug, UserExperience, Rating, Feature>", "confidence": float between 0 and 1, "reason": "your refined explanation"}}
        """
